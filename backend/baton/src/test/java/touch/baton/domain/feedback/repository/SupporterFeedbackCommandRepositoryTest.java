...
class SupporterFeedbackCommandRepositoryTest extends RepositoryTestConfig {

    @Autowired
    private EntityManager em;
    @Autowired
    private SupporterFeedbackCommandRepository supporterFeedbackCommandRepository;
    ...
    void existsByRunnerPostIdAndSupporterId() {
        ...
        // when, then
        assertSoftly(softly -> {
            softly.assertThat(supporterFeedbackCommandRepository.existsByRunnerPostIdAndSupporterId(runnerPost.getId(), reviewedSupporter.getId())).isTrue();
            softly.assertThat(supporterFeedbackCommandRepository.existsByRunnerPostIdAndSupporterId(runnerPost.getId(), notReviewedSupporter.getId())).isFalse();
        });
    }
}
